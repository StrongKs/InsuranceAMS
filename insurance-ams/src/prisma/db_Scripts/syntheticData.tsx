        ...
        Array.from({ length: numClients }).map(async () => {
            const newClient = await prisma.client.create({
                data: {
                    ...
                    dependants: faker.helpers.arrayElements([faker.person.firstName(), faker.person.firstName()], 2),
                    stage: faker.helpers.arrayElement(["LEAD", "CONSULT", "QUOTE", "CLOSED_ACCEPTED", "CLOSED_DENIED"]),
        ...
        Array.from({ length: numberOfPolicies }).map(() => {
            ...
            return prisma.policy.create({
                data: {
                    ...
                    premium: parseFloat(faker.finance.amount({ min: 100, max: 1000, dec: 2 })),
                    isActive: faker.datatype.boolean(),
                    status: faker.helpers.arrayElement([PolicyStatus.PENDING_SIGNATURE, PolicyStatus.ACTIVE, PolicyStatus.CANCELLED]),
        ...
        Array.from({ length: numClaims }).map(() => {
            ...
            return prisma.claim.create({
                data: {
                    ...
                    status: faker.helpers.arrayElement([ClaimStatus.PENDING, ClaimStatus.APPROVED, ClaimStatus.REJECTED, ClaimStatus.IN_REVIEW, ClaimStatus.CLOSED]),
